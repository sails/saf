...
void ServerStatProcessor::serverstat(sails::net::HttpRequest& request,
  ...
  for (size_t i = 0; i < server->NetThreadNum(); i++) {
    ...
    net::NetThread<net::PacketCommon, HandleImpl>::NetThreadStatus netstatus = server->GetNetThreadStatus(i);

    netthread_dict->SetIntValue("NetThread_NO", i);
    netthread_dict->SetIntValue("NetThread_status", netstatus.status);
    netthread_dict->SetIntValue("NetThread_connector_num", netstatus.connector_num);
    netthread_dict->SetIntValue("NetThread_accept_times", netstatus.accept_times);
    netthread_dict->SetIntValue("NetThread_recv_queue_capacity", netstatus.recv_queue_capacity);
    netthread_dict->SetIntValue("NetThread_recv_queue_size", netstatus.recv_queue_size);
    netthread_dict->SetIntValue("NetThread_send_queue_capacity", netstatus.send_queue_capacity);
    netthread_dict->SetIntValue("NetThread_send_queue_size", netstatus.send_queue_size);
  }
  ...
  for (size_t i = 0; i < server->HandleThreadNum(); i++) {
    ...
    net::HandleThread<net::PacketCommon, HandleImpl>::HandleThreadStatus handlestatus = server->GetHandleThreadStatus(i);

    handlethread_dict->SetIntValue("HandleThread_NO", i);
    handlethread_dict->SetIntValue("HandleThread_status", handlestatus.status);
    handlethread_dict->SetIntValue("HandleThread_handle_times", handlestatus.handle_times);
    handlethread_dict->SetIntValue("HandleThread_handle_queue_capacity", handlestatus.handle_queue_capacity);
    handlethread_dict->SetIntValue("HandleThread_handle_queue_size", handlestatus.handle_queue_size);
  }
