...
bool login_check(const LoginRequest *request) {
    ...
    char param[500] = {'\0'};
    if (config.is_test()) {
      sprintf(param,"model=netgame&action=authuser&uid=%s&ticket=%s&time=%s&sign=%s&line=10", request->username().c_str(), base::url_encode(password.c_str(),password_encoding) ,time_str,strsignMd5.c_str());

    }else {
	sprintf(param,"model=netgame&action=authuser&uid=%s&ticket=%s&time=%s&sign=%s", request->username().c_str(),base::url_encode(password.c_str(),password_encoding) ,time_str,strsignMd5.c_str());
